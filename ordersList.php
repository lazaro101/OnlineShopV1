...
<div class="content">
	...
	<ul class="nav nav-tabs">
  		<li role="presentation" class="active"><a href="ordersList.php">All Orders</a></li>
  		<li role="presentation"><a href="ordersListFull.php">Delivered</a></li>
  		<li role="presentation"><a href="ordersListUnful.php">Processing</a></li>
...
<!-- 		<div class="form-group col-md-4">
    ...
    <div class="table-container col-md-12" >
		...
		<thead>
			...
			<?php
				$sql = "SELECT orders.orderID, orders.orderDate, orders.status, orders.subTotal, client.firstName, client.lastName 
						FROM orders 
						INNER JOIN client 
						ON orders.username = client.email AND orders.deleted = '0' AND orders.checkout = '1'
			...
			<tr onclick="window.document.location.href='editOrdersList.php?varname=<?php echo $row["orderID"]; ?> ';" style="cursor: pointer;">
				<td class="chk" onclick="if (event.stopPropagation) {event.stopPropagation;}event.cancelBubble = true;return true;">
					<div class="checkbox">
					  <label>
					    <input type="checkbox" value="<?php echo $row['orderID'];?>" aria-label="..." name="chkbox[]">
					  </label>
					</div>
				</td>
				...
				<td class="sta">
					<?php if($row['status'] == 0) { 
							echo '<span class="bg-warning stat">Processing</span>'; 
						} elseif($row['status'] == 1) { 
							echo '<span class="bg-success stat">Delivered</span>'; 
						} else {
							echo '<span class="bg-danger stat">Cancelled</span>'; 
							}?>	
				</td>
		...
		<button type="submit" class="delbtn btn btn-default hidden" name="delMultiOrder" id="submit-form">Archive</button>
	</div>
	</form>
</div>
